    ...
    searRoleObj = {
        search: function () {
            $('#role_list').datagrid('load', {
                ...
                role_url: $('input[name="search_role_url"]').val(),
                role_id: $("input[name='search_role_pid']").val()
            });
        }
    };
    //datagrid初始化
...
function openModifyRoleWin(roleId) {
    ...
    $('#modify_role').window({
        ...
        minimizable: false,
        collapsible: false,//是否可折叠的
        href: "/role/updateview?role_id=" + roleId
    });
}
